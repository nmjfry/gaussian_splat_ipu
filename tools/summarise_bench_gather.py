...
discovery_ms, stream_ms, run_ms, readback_ms, total_ms, assigned) plus optional
...
def main():
    ...
    for scene in scenes:
        ...
        run  = tail(load_floats(prof, "run_ms"))
    ...
    hdr = ("{:>10s}  {:>6s}  {:>9s}  {:>6s}  {:>10s}  {:>8s}  {:>7s}  {:>9s}  {:>9s}  {:>8s}")
    rfmt = ("{:>10s}  {:>6d}  {:>9.2f}  {:>6.2f}  {:>10.2f}  {:>8.2f}  {:>7.2f}  {:>9.2f}  {:>9.0f}  {:>8.2f}")
    print(hdr.format("scene", "frames", "total_ms", "fps", "discovery", "stream", "run",
                     "readback", "assigned", "power_W"))
    print("-" * 110)
    for r in rows:
        print(rfmt.format(r["scene"], r["frames"], r["total_ms_mean"], r["fps_mean"],
                          r["discovery_ms_mean"], r["stream_ms_mean"], r["run_ms_mean"],
    ...
    if rows:
        ...
        print(rfmt.format("avg", 0, avg("total_ms_mean"), avg("fps_mean"),
                          avg("discovery_ms_mean"), avg("stream_ms_mean"), avg("run_ms_mean"),
